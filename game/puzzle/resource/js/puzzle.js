function Puzzle(selector) {
    ...
    var _self = this,   //当前对象的引用
        ...
        moveHandlerName = canTouch ? "touchstart" : "click",
    ...
    init = function(parm) { //构建game调用的方法   参数如为字符串则认为是图片路径，其余属性均应用默认设置
        ...
        /* 获取矩阵下标信息集合 */
        positions = getPosition(size);
        _self.size = size;
        // !!这个上线时要删掉，测试兼容模式
        // realData = "1,2,3,4,5,6,7,8,9";
        // !!结束
        /* 动态创建碎片并定位至容器内 */
        ...
        /* 给碎片绑定点击事件 */
        useHigh ? $('.puzzle-item').on(moveHandlerName + ".move", function() {
            if (gameStatus === 1) calculate($(this).data('index'), size);
        }) : $('.puzzle-item img').on(moveHandlerName + ".move", function() {
            if (gameStatus === 1 && !onMove) calculate($(this).parent().data('index'), size);
        })

        parm['autoplay'] && fireEvent('start'); //直接开始
        parm['timing'] && timer();              //计时
        /* 根据浏览器而创建的两套不同的方案 */
        function lowMatrix($item, position) {
            $item.css({
                'overflow': 'hidden'
        ...
        function highMatrix($item, position) {
            $item.css({
        ...
        function highPosition(left, top, index, rowIndex, x, y) {
            return {
                'transform': 'translate3d(' + (left + gap * (1 + index)) + 'px, ' + (top + gap * (1 + rowIndex)) + 'px, 0)',
                '-webkit-transform': 'translate3d(' + (left + gap * (1 + index)) + 'px, ' + (top + gap * (1 + rowIndex)) + 'px, 0)',
                '-ms-transform': 'translate3d(' + (left + gap * (1 + index)) + 'px, ' + (top + gap * (1 + rowIndex)) + 'px, 0)',
                '-o-transform': 'translate3d(' + (left + gap * (1 + index)) + 'px, ' + (top + gap * (1 + rowIndex)) + 'px, 0)',
        ...
        function highCoordinate($item) {
            ...
            return (_style.transform || _style.webkitTransform || _style.mozTransform || _style.msTransform || _style.oTransform).replace("translate3d","").match(/\d+(\.\d+)?/g);
        }
    ...
    */
    ...
    function resize (_width) {
        ...
        for (var rowIndex = 0; rowIndex < size; rowIndex++) {
            ...
            for (var index = 0, size = row.length; 
                ...
                var item = row[index];
                var position = positions[rowIndex][index];
                var $item = $("#item-" + item).css({
                    'width': itemWidth,
                    'height': itemHeight
                });
                $item && buildMatrix($item, position);
            }
        }
    }
    /* 记步 */
    ...
    _self.timer = timer;
    _self.resize = resize;
    _self.getGameStatus = getGameStatus;
